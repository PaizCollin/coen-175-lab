 ...
 *		longs,and void types. Return type specifier
 *
 *		specifier:
 *		  int
 *		  char
 ...
 * 		Returns number of levels of indirection
 *
 *		pointers:
 *		  empty
 *		  * pointers
...
static void declarator(int typespec)
{
    ...
    if (lookahead == '[') {
		match('[');
		unsigned long length = stoul(lexbuf);
		match(NUM);
		match(']');
		Type type(typespec, indirection, length);
		declareVariable(temp, type);
    } else {
 ...
 *		closing parentheses. Return parameters
 *
 *		parameters:
 *		  void
...
static void globalOrFunction()
{
    ...
    if (lookahead == '[') {
		...
		if (lookahead == ')') {
			...
			Type type(typespec, indirection, param_list);
			match(')');
			defineFunction(temp, type);
			match('{');
			declarations();
			statements();
			match('}');
